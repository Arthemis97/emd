...
const useBlankStore = defineStore("blank", {
    ...
    actions: {
        ...
        async storeBlank(data) {
            ...
            if (resp.resp.status === 200) {
                message.success(resp.data?.message || "Загвар нэмэгдлээ");
        ...
        async updateBlank(data) {
            ...
            if (resp.resp.status === 200) {
                message.success(resp.data?.message || "Загвар засагдлаа");
                const index = this.blanks.findIndex(
                    (item) => item.id === resp.data.blank.id
                );
                if (index !== -1) {
                    const updatedBlanks = [
        ...
        async deleteBlank(id) {
            ...
            if (resp.resp.status === 200) {
                message.success(resp.data?.message || "Загвар устгагдлаа");
